...
// 直接量赋值，b被重新赋值，b指向的value为2，其所占空间由垃圾回收器回收，新的b指向a的引用值
...
d[0] = 'a';
console.log(c, d);
// 变量指向一个引用对象，c本身占空间，其指向的数组是个引用空间，存储结构是数组，里面包含的value是1和2。
// d指向了c的引用对象，改变时会同时影响到c与d，它们指向的是同一个引用空间。
...
d[0] = 'a';
console.log(c, d);
// 支持连续多个变量赋值，d = c, c = 空对象，c与d相同，指向共同的引用对象。
var a = { a: a };
console.log(a);
a.b = a = { x: 2 };
console.log(a, a.b);
// 连续赋值从左到右解释执行。首先a.b 赋值为a, 变成了一个循环引用，然后将a赋值为新的引用对象。
function foo() {
    var a = b = 1;
    console.log(a, b);
}
...
foo();
foo()();
// 表达式声明函数不会提前，但是num1变量声明会提前，故foo执行时返回undefined
...
// arguemnts可以被修改。arguemnts[0]与内部的a是指向同一位置。外部的a仍然原始值。
...
/* # assignment and reference value with function */
foo();
function foo() {
...
foo1();
// foo是定义式声明，foo2是表达式声明, foo预先定义, foo2调用时还没有赋值。
// foo名称是foo, foo2是个匿名函数，赋值给foo2变量，name沿用赋值时的变量名，foo1不存在。
...
/* ##################### */
/* Part 2: The puzzlement of language */
/* ##################### */
